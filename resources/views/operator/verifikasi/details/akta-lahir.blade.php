...
<head>
    ...
    <title>Review Akta Lahir - SINDESA</title>
...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    @php
        // Decode data JSON dari kolom data_tambahan
    ...
    <aside id="sidebar"
        class="fixed inset-y-0 left-0 w-[280px] bg-gradient-to-b from-[#11442b] to-[#1a5e35] text-white transition-transform duration-300 transform -translate-x-full lg:translate-x-0 z-[1000] flex flex-col shadow-xl">

        <div class="p-8 text-center border-b border-white/10 flex justify-center">
        ...
        <nav class="flex-1 px-4 pb-4 space-y-1 overflow-y-auto">
            <a href="{{ route('operator.dashboard') }}"
                class="flex items-center gap-3 p-3 {{ request()->routeIs('operator.dashboard') ? 'bg-[#cfa03f] text-white shadow-md' : 'text-white/80 hover:bg-[#cfa03f] hover:text-white' }} rounded-lg transition-all mb-2">
            ...
            <a href="{{ route('operator.menunggu-ttd') }}"
                class="flex items-center gap-3 p-3 {{ request()->routeIs('operator.menunggu-ttd') ? 'bg-[#cfa03f] text-white' : 'text-white/80 hover:bg-[#cfa03f] hover:text-white' }} rounded-lg transition-all mb-2">
    ...
    <main class="flex-1 p-6 lg:p-8 lg:ml-[280px] overflow-x-hidden min-h-screen">
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h2 class="text-2xl font-bold text-[#1a5e35]">Review Detail Akta Lahir</h2>
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">ID Pengajuan:
                    #AKTA-{{ str_pad($surat->id, 6, '0', STR_PAD_LEFT) }}</p>
            </div>
        </div>
                        ...
                        <div
                            ...
                            <div class="flex-1">
                                ...
                                <span class="text-sm text-gray-800 font-medium">
                                    {{ !empty($dataTambahan[$key]) ? 'Berkas Tersedia' : 'Berkas Tidak Ditemukan' }}
                                </span>
                            </div>
                            @if(!empty($dataTambahan[$key]))
                                <a href="{{ asset('storage/' . $dataTambahan[$key]) }}" target="_blank"
