...
<head>
    ...
    <title>Tanda Tangan Izin Keramaian - SINDESA</title>
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">
                    Pengantar Izin Keramaian - {{ $surat->nomor_surat ?? 'NOMOR BELUM DISET' }}
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm">Surat ini telah diverifikasi keabsahannya oleh Operator pada
                    {{ $surat->updated_at->translatedFormat('d F Y, H:i') }} WIB. Dokumen pengantar RT/RW dan identitas
                    penanggung jawab dinyatakan valid.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] border border-gray-100 p-6 md:p-10">
            ...
            <div class="mb-10">
                <h3
                    ...
                    <i class="fas fa-user-shield"></i> Identitas Penanggung Jawab Acara
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-1">
                        <label
                            class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">NIK</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['nik_penanggung_jawab'] ?? '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Nama
                            Lengkap</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['nama_penanggung_jawab'] ?? '-' }}
                        </div>
                    </div>
                    <div>
            ...
            <div class="mb-10">
                <h3
                    ...
                    <i class="fas fa-calendar-alt"></i> Rincian Acara Keramaian
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Jenis
                            Acara / Keramaian</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-orange-800 font-bold text-lg">
                            {{ $dataPengajuan['jenis_acara'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tanggal
                        ...
                        <div
                            ...
                            {{ !empty($dataPengajuan['tanggal_mulai']) ? \Carbon\Carbon::parse($dataPengajuan['tanggal_mulai'])->translatedFormat('d F Y') : '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tanggal
                        ...
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-semibold">
                            {{ !empty($dataPengajuan['tanggal_selesai']) ? \Carbon\Carbon::parse($dataPengajuan['tanggal_selesai'])->translatedFormat('d F Y') : '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Lokasi
                            Acara Berlangsung</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-semibold leading-relaxed">
                            {{ $dataPengajuan['lokasi_acara'] ?? '-' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-10">
                <h3 class="flex items-center gap-2 text-lg font-bold text-red-600 border-b-2 border-gray-50 pb-3 mb-6">
                    <i class="fas fa-exclamation-triangle"></i> Pernyataan Penanggung Jawab
                </h3>
                <div class="bg-red-50 p-6 rounded-xl border border-red-200 flex items-start gap-4">
                    <i class="fas fa-check-circle text-2xl text-green-600 mt-1"></i>
                    <p class="text-sm text-gray-800 font-medium leading-relaxed">
                        Penanggung jawab acara <span class="font-bold">bersedia menjaga keamanan dan ketertiban</span>.
                        Menjamin <span class="text-red-600 font-bold">tidak akan ada aktivitas Pornografi, Porno Aksi /
                            Candoleng-doleng, Perjudian, dan mengkonsumsi minuman keras (Miras)</span> yang bertentangan
                        dengan hukum selama acara berlangsung.
                    </p>
                </div>
            </div>
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <label
                                ...
                                RT / RW</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_pengantar_rt']) ? 'Berkas Tersedia' : 'Berkas Tidak Ditemukan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_pengantar_rt']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_pengantar_rt']) }}" target="_blank"
                                class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                                <i class="fas fa-cloud-download-alt"></i> Lihat File
                            </a>
                        @else
