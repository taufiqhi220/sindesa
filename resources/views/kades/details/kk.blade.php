...
<head>
    ...
    <title>Tanda Tangan Pengantar KK - SINDESA</title>
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">
                    Pengantar KK - {{ $surat->nomor_surat ?? 'NOMOR BELUM DISET' }}
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm">Surat ini telah diverifikasi oleh Operator pada
                    {{ $surat->updated_at->translatedFormat('d F Y, H:i') }} WIB. Pengajuan Kartu Keluarga dan
                    kelengkapan berkas telah diperiksa sah.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] border border-gray-100 p-6 md:p-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div
                            ...
                            {{ $dataPengajuan['tujuan_pengajuan'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label
                            class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Keperluan
                            Surat</label>
                        <div
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Nomor
                            Kartu Keluarga (KK Lama)</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['kk_lama'] ?? '- Tidak Ada / Pembuatan Baru -' }}
                        </div>
                    </div>
            ...
            <div class="mb-10">
                <h3 class="flex items-center gap-2 text-lg font-bold text-blue-600 border-b-2 border-gray-50 pb-3 mb-6">
                    <i class="fas fa-home"></i> Detail Kartu Keluarga (Tujuan)
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-blue-50 p-6 rounded-xl border border-blue-100">
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-blue-700 uppercase tracking-widest mb-1">Nama
                            Kepala Keluarga (Tujuan)</label>
                        <div class="text-blue-900 font-bold text-lg">{{ $dataPengajuan['nama_kepala_keluarga'] ?? '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-blue-700 uppercase tracking-widest mb-1">Alamat
                            (Dusun/Jalan)</label>
                        <div class="text-blue-900 font-semibold">{{ $dataPengajuan['alamat'] ?? '-' }}</div>
                    </div>
                    <div>
                        <label
                            class="block text-[10px] font-bold text-blue-700 uppercase tracking-widest mb-1">RT</label>
                        <div class="text-blue-900 font-semibold">{{ $dataPengajuan['rt'] ?? '-' }}</div>
                    </div>
                    <div>
                        <label
                            class="block text-[10px] font-bold text-blue-700 uppercase tracking-widest mb-1">RW</label>
                        <div class="text-blue-900 font-semibold">{{ $dataPengajuan['rw'] ?? '-' }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-10">
                ...
                <div class="space-y-4">
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Buku
                                Nikah / Akta Cerai (Jika ada perubahan status)</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_nikah']) ? 'Berkas Tersedia' : 'Berkas Tidak Dilampirkan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_nikah']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_nikah']) }}" target="_blank"
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">KK
                                Lama</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ (!empty($dataPengajuan['file_kk_lama'])) ? 'Berkas Tersedia' : 'Berkas Tidak Dilampirkan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_kk_lama']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_kk_lama']) }}" target="_blank"
                                class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                                <i class="fas fa-cloud-download-alt"></i> Lihat File
                            </a>
                        @else
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <label
                                class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Surat
                                Pindah / Dokumen Lainnya</label>
                            <span
