...
<head>
    ...
    <title>Tanda Tangan Ket. Kematian - SINDESA</title>
...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    <main class="flex-1 p-6 lg:p-8 lg:ml-[280px] overflow-x-hidden min-h-screen">
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">
                    Keterangan Kematian - {{ $surat->nomor_surat ?? 'NOMOR BELUM DISET' }}
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm">Surat ini telah diverifikasi oleh Operator pada
                    {{ $surat->updated_at->translatedFormat('d F Y, H:i') }} WIB. Identitas Almarhum/ah, pelapor, dan
                    keterangan rumah sakit/saksi telah diperiksa silang.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] border border-gray-100 p-6 md:p-10">
            ...
            <div class="mb-10">
                <h3
                    class="flex items-center gap-2 text-lg font-bold text-[#1a5e35] border-b-2 border-gray-50 pb-3 mb-4">
                    <i class="fas fa-clipboard-list"></i> Tujuan & Keperluan Surat
                </h3>
                <div>
                    <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Keperluan
                        Pengajuan</label>
                    <div
                        class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold leading-relaxed">
                        {{ $surat->keperluan ?? 'Tidak dicantumkan oleh pemohon.' }}
                    </div>
                </div>
            </div>
            ...
            <div class="mb-10">
                <h3
                    ...
                    <i class="fas fa-user-alt-slash"></i> Identitas Almarhum / Almarhumah
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tempat
                            Lahir</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['tempat_lahir_almarhum'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tanggal
                            Lahir</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ !empty($dataPengajuan['tanggal_lahir_almarhum']) ? \Carbon\Carbon::parse($dataPengajuan['tanggal_lahir_almarhum'])->translatedFormat('d F Y') : '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Jenis
                            Kelamin</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['jenis_kelamin_almarhum'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Status
                            Perkawinan</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['status_perkawinan_almarhum'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label
                            class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Kewarganegaraan</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['kewarganegaraan_almarhum'] ?? '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        <label
                        ...
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['pekerjaan_almarhum'] ?? '-' }}
                        </div>
                    </div>
            ...
            <div class="mb-10">
                <h3 class="flex items-center gap-2 text-lg font-bold text-red-600 border-b-2 border-red-50 pb-3 mb-6">
                    <i class="fas fa-heartbeat"></i> Rincian Waktu & Sebab Kematian
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 bg-red-50 p-6 rounded-xl border border-red-100">
                    <div class="md:col-span-1">
                        <label class="block text-[10px] font-bold text-red-700 uppercase tracking-widest mb-1">Tanggal
                            Kematian</label>
                        <div class="text-red-900 font-bold text-lg">
                            {{ !empty($dataPengajuan['tanggal_kematian']) ? \Carbon\Carbon::parse($dataPengajuan['tanggal_kematian'])->translatedFormat('d F Y') : '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-1">
                        <label class="block text-[10px] font-bold text-red-700 uppercase tracking-widest mb-1">Umur
                            (Tahun)</label>
                        <div class="text-red-900 font-semibold mt-1">{{ $dataPengajuan['umur_kematian'] ?? '-' }} Tahun
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-red-700 uppercase tracking-widest mb-1">Sebab
                            Kematian</label>
                        <div class="text-red-900 font-semibold mt-1">{{ $dataPengajuan['sebab_kematian'] ?? '-' }}</div>
                    </div>
                    <div class="md:col-span-4">
                        <label class="block text-[10px] font-bold text-red-700 uppercase tracking-widest mb-1">Tempat
                            Meninggal</label>
                        <div class="text-red-900 font-semibold mt-1">{{ $dataPengajuan['tempat_kematian'] ?? '-' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-10">
                <h3
                    class="flex items-center gap-2 text-lg font-bold text-[#cfa03f] border-b-2 border-gray-50 pb-3 mb-6">
                    <i class="fas fa-user-edit"></i> Identitas Pelapor
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Nama
                            Pelapor</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-orange-800 font-bold">
                            {{ $dataPengajuan['nama_pelapor'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Hubungan
                            dengan Almarhum/ah</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-orange-800 font-semibold">
                            {{ $dataPengajuan['hubungan_pelapor'] ?? '-' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-10">
                ...
                <div class="space-y-4">
                    <div
                        ...
                        <div class="flex-1">
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">KTP
                                Almarhum</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_ktp_almarhum']) ? 'Berkas Tersedia' : 'Berkas Tidak Ditemukan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_ktp_almarhum']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_ktp_almarhum']) }}" target="_blank"
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">KK
                                Almarhum</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_kk_almarhum']) ? 'Berkas Tersedia' : 'Berkas Tidak Ditemukan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_kk_almarhum']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_kk_almarhum']) }}" target="_blank"
                                class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                                <i class="fas fa-cloud-download-alt"></i> Lihat File
                            </a>
                        @else
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">KTP
                                Pelapor</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_ktp_pelapor']) ? 'Berkas Tersedia' : 'Berkas Tidak Ditemukan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_ktp_pelapor']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_ktp_pelapor']) }}" target="_blank"
                                class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                                <i class="fas fa-cloud-download-alt"></i> Lihat File
                            </a>
                        @else
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <label
                                class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Keterangan
                                Rumah Sakit / Puskesmas (Opsional)</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_rs']) ? 'Berkas Tersedia' : 'Berkas Tidak Dilampirkan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_rs']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_rs']) }}" target="_blank"
                                class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                                <i class="fas fa-cloud-download-alt"></i> Lihat File
                            </a>
                        @else
