...
<head>
    ...
    <title>Detail Beda Nama - SINDESA</title>
...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    @php
        $dataPengajuan = is_array($surat->data_tambahan) ? $surat->data_tambahan : (json_decode($surat->data_tambahan, true) ?? []);
    @endphp
    ...
    <main class="flex-1 p-6 lg:p-8 lg:ml-[280px] overflow-x-hidden min-h-screen">
        @php
            // Memastikan JSON ter-decode dengan benar jadi array
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">
                    Keterangan Beda Nama - {{ $surat->nomor_surat ?? 'NOMOR BELUM DISET' }}
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm">Surat ini telah diverifikasi keabsahannya oleh Operator pada
                    {{ $surat->updated_at->translatedFormat('d F Y, H:i') }} WIB. Seluruh berkas dinyatakan lengkap dan
                    perbedaan identitas telah dikonfirmasi valid.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] border border-gray-100 p-6 md:p-10">
            ...
            <div class="mb-10">
                <h3
                    ...
                    <i class="fas fa-id-card"></i> Data Pada Dokumen 1 (KTP / KK)
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tempat
                            Lahir</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['tempat_lahir_dok1'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tanggal
                            Lahir</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ !empty($dataPengajuan['tanggal_lahir_dok1']) ? \Carbon\Carbon::parse($dataPengajuan['tanggal_lahir_dok1'])->translatedFormat('d F Y') : '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Jenis
                            Kelamin</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['jenis_kelamin_dok1'] ?? '-' }}
                        </div>
                    </div>
            ...
            <div class="mb-10">
                <h3
                    ...
                    <i class="fas fa-file-alt"></i> Data Pada Dokumen 2 (Yang Berbeda)
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-1">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Nama
                        ...
                        <div
                            ...
                            {{ $dataPengajuan['nama_dokumen2'] ?? '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-1">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">NIK /
                            No. Identitas Lain</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['nomor_dok2'] ?? '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-1">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Nama
                            Lengkap (Di Dokumen 2)</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-red-600 font-semibold">
                            {{ $dataPengajuan['nama_dok2'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tempat
                        ...
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['tempat_lahir_dok2'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tanggal
                        ...
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-semibold">
                            {{ !empty($dataPengajuan['tanggal_lahir_dok2']) ? \Carbon\Carbon::parse($dataPengajuan['tanggal_lahir_dok2'])->translatedFormat('d F Y') : '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Jenis
                        ...
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['jenis_kelamin_dok2'] ?? '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-3">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Alamat
                            Lengkap</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-semibold leading-relaxed">
                            {{ $dataPengajuan['alamat_dok2'] ?? '-' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-10">
                <h3 class="flex items-center gap-2 text-lg font-bold text-blue-600 border-b-2 border-gray-50 pb-3 mb-6">
                    <i class="fas fa-check-double"></i> Rincian Data yang Benar
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-blue-50 p-6 rounded-xl border border-blue-100">
                    <div>
                        <label class="block text-[10px] font-bold text-blue-700 uppercase tracking-widest mb-1">Data
                            yang Berbeda</label>
                        <div class="text-blue-900 font-bold">{{ $dataPengajuan['data_berbeda'] ?? '-' }}</div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-blue-700 uppercase tracking-widest mb-1">Acuan
                            Kebenaran</label>
                        <div class="text-blue-900 font-bold">
                            {{ $dataPengajuan['acuan_kebenaran'] ?? 'Dokumen 1 (KTP/KK)' }}
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-4">
                    <div
                        ...
                        <div class="flex-1">
                            <label
                                ...
                                1 (KTP/KK)</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_dok1']) ? 'Berkas Tersedia' : 'Berkas Tidak Ditemukan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_dok1']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_dok1']) }}" target="_blank"
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <label
                                class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Dokumen
                                2 (Ijazah/Akte/dll)</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_dok2']) ? 'Berkas Tersedia' : 'Berkas Tidak Ditemukan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_dok2']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_dok2']) }}" target="_blank"
                                class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                                <i class="fas fa-cloud-download-alt"></i> Lihat File
                            </a>
                        @else
