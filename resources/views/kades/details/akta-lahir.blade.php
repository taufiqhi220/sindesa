...
<head>
    ...
    <title>Tanda Tangan Akta Lahir - SINDESA</title>
...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    <aside id="sidebar"
        ...
        <nav class="flex-1 px-4 pb-4 space-y-1 overflow-y-auto custom-scrollbar">
            ...
            <a href="{{ route('kades.perlu-ttd') }}"
                ...
                @if(isset($countPerluTtd) && $countPerluTtd > 0)
    ...
    <main class="flex-1 p-6 lg:p-8 lg:ml-[280px] overflow-x-hidden min-h-screen">
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">
                    ...
                    {{ $surat->nomor_surat ?? 'NOMOR SURAT BELUM DISET' }}
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <i class="fas fa-shield-check text-2xl text-emerald-600 mt-0.5"></i>
            <div>
                <strong class="block mb-1 text-emerald-700">Siap Ditandatangani</strong>
                <p class="text-sm">Berkas pengajuan ini telah diverifikasi dan dinyatakan lengkap oleh staf Operator
                    Desa pada {{ $surat->updated_at->translatedFormat('d F Y, H:i') }} WIB.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] border border-gray-100 p-6 md:p-10">
            ...
            <div class="mb-10">
                <h3
                    class="flex items-center gap-2 text-lg font-bold text-[#1a5e35] border-b-2 border-gray-50 pb-3 mb-6 font-semibold">
                    <i class="fas fa-child"></i> Identitas Anak / Pemohon
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tempat
                            Lahir</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $surat->data_tambahan['tempat_lahir_anak'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tanggal
                            Lahir</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ isset($surat->data_tambahan['tanggal_lahir_anak']) ? \Carbon\Carbon::parse($surat->data_tambahan['tanggal_lahir_anak'])->translatedFormat('d F Y') : '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Jenis
                            Kelamin</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ ($surat->data_tambahan['jenis_kelamin_anak'] ?? '-') }}
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $surat->data_tambahan['agama_anak'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label
                        ...
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $surat->data_tambahan['kewarganegaraan_anak'] ?? '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        ...
                        <div
                            ...
                            {{ $surat->data_tambahan['alamat_anak'] ?? '-' }}
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-4">
                    ...
                    @foreach($lampiran as $key => $label)
                        <div
                            ...
                            <div class="flex-1">
                                <label
                                    class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{{ $label }}</label>
                                <span
                                    class="text-sm text-gray-800 font-medium">{{ !empty($surat->data_tambahan[$key]) ? 'Berkas Tersedia' : 'Berkas Tidak Ditemukan' }}</span>
                            </div>
                            @if(!empty($surat->data_tambahan[$key]))
                                <a href="{{ asset('storage/' . $surat->data_tambahan[$key]) }}" target="_blank"
                ...
                <div class="bg-gray-50 rounded-2xl p-6 md:p-8 border border-gray-100 shadow-inner mt-12">
                    <div class="mb-6">
                        ...
                        <textarea name="keterangan" id="catatanPenolakan"
                            placeholder="Wajib diisi jika berkas ini ditolak dan dikembalikan..." rows="3" required
                    ...
                    <div class="flex flex-col sm:flex-row justify-end gap-4">
                        <button type="submit" onclick="return validasiTolak()"
                            ...
                            <i class="fas fa-times-circle text-sm"></i> Tolak Dokumen
                        </button>
                        <button type="button" onclick="openPrintModal()"
                            class="px-8 py-3 bg-[#cfa03f] text-white rounded-xl font-bold hover:bg-[#b88e32] transition-all flex items-center justify-center gap-2 shadow-lg uppercase text-xs tracking-wider">
                            <i class="fas fa-file-signature text-sm"></i> Setujui & Tanda Tangani
                        </button>
                    </div>
                </div>
            </form>
