...
<head>
    ...
    <title>Tanda Tangan SKTM - SINDESA</title>
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">
                    Keterangan Tidak Mampu - {{ $surat->nomor_surat ?? 'NOMOR BELUM DISET' }}
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm">Surat ini telah diverifikasi oleh Operator pada
                    {{ $surat->updated_at->translatedFormat('d F Y, H:i') }} WIB. Berkas pengantar dari Kepala Dusun
                    telah dilampirkan dan sesuai dengan kriteria keluarga pra-sejahtera.
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] border border-gray-100 p-6 md:p-10">
            ...
            <div class="mb-10">
                <h3
                    ...
                    <i class="fas fa-user"></i> Identitas Yang Bersangkutan (Pemohon)
                </h3>
            ...
            <div class="mb-10">
                <h3
                    ...
                    <i class="fas fa-users"></i> Identitas Kepala Keluarga (Orang Tua/Wali)
                </h3>
                <div
                    class="grid grid-cols-1 md:grid-cols-3 gap-6 bg-orange-50/30 p-6 rounded-xl border border-orange-100/50">
                    <div class="md:col-span-1">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">NIK
                            Kepala Keluarga</label>
                        <div class="px-4 py-3 bg-white border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['nik_kepala_keluarga'] ?? '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Nama
                            Kepala Keluarga</label>
                        <div
                            class="px-4 py-3 bg-white border border-gray-100 rounded-xl text-gray-800 font-bold text-lg">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tanggal
                            Lahir</label>
                        <div class="px-4 py-3 bg-white border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ !empty($dataPengajuan['tanggal_lahir_kk']) ? \Carbon\Carbon::parse($dataPengajuan['tanggal_lahir_kk'])->translatedFormat('d F Y') : '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Jenis
                            Kelamin</label>
                        <div class="px-4 py-3 bg-white border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['jenis_kelamin_kk'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label
                    ...
                    <div class="md:col-span-3">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Alamat
                            Kepala Keluarga</label>
                        <div
                            class="px-4 py-3 bg-white border border-gray-100 rounded-xl text-gray-800 font-semibold leading-relaxed">
                            {{ $dataPengajuan['alamat_kk'] ?? '-' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-10">
                <h3 class="flex items-center gap-2 text-lg font-bold text-blue-600 border-b-2 border-gray-50 pb-3 mb-6">
                    <i class="fas fa-file-invoice"></i> Keperluan SKTM
                </h3>
                <div class="mb-6 bg-blue-50 p-6 rounded-xl border border-blue-100">
                    <label class="block text-[10px] font-bold text-blue-700 uppercase tracking-widest mb-1">Keperluan
                        Pembuatan</label>
                    <div class="text-blue-900 font-bold text-lg uppercase">
                        {{ $dataPengajuan['keperluan'] ?? $surat->keperluan ?? '-' }}
                    </div>
                </div>
                <div class="flex items-start gap-4 p-4 bg-red-50 rounded-xl border border-red-100">
                    ...
                    <p class="text-sm text-red-800 font-medium">Pemohon menyatakan dengan sadar bahwa keluarga tergolong
                        Tidak Mampu (Pra-Sejahtera) dan bersedia dituntut jika data tidak benar.</p>
                </div>
            </div>
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            <label
                                ...
                                Kepala Dusun / RT</label>
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_pengantar']) ? 'Berkas Tersedia' : 'Berkas Tidak Dilampirkan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_pengantar']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_pengantar']) }}" target="_blank"
                                class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                                <i class="fas fa-cloud-download-alt"></i> Lihat File
                            </a>
                        @else
