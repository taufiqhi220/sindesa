...
<head>
    ...
    <title>Tanda Tangan Ket. Janda/Duda - SINDESA</title>
    ...
    <main class="flex-1 p-6 lg:p-8 lg:ml-[280px] overflow-x-hidden min-h-screen">
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 font-medium uppercase tracking-wide">
                    Keterangan Janda/Duda - {{ $surat->nomor_surat ?? 'NOMOR BELUM DISET' }}
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm">Surat ini telah diverifikasi oleh Operator. Bukti dokumen (Akta Cerai/Surat Kematian)
                    telah diverifikasi dan sesuai dengan data yang diajukan.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] border border-gray-100 p-6 md:p-10">
            ...
            <div class="mb-10">
                <h3
                    ...
                    <i class="fas fa-user-check"></i> Identitas Pemohon
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tempat
                            Lahir</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['tempat_lahir_pemohon'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tanggal
                            Lahir</label>
                        <div class="px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-gray-800 font-semibold">
                            {{ !empty($dataPengajuan['tanggal_lahir_pemohon']) ? \Carbon\Carbon::parse($dataPengajuan['tanggal_lahir_pemohon'])->translatedFormat('d F Y') : '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Penyebab
                            Status</label>
                        <div
                            class="px-4 py-3 bg-red-50 border border-red-100 rounded-xl text-red-700 font-bold uppercase text-xs">
                            {{ $dataPengajuan['penyebab_status'] ?? '-' }}
                        </div>
                    </div>
            ...
            <div class="mb-10">
                <h3
                    ...
                    <i class="fas fa-user-times"></i> Data Mantan Pasangan (Suami/Istri)
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Nama
                            Mantan Suami/Istri</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-semibold">
                            {{ $dataPengajuan['nama_mantan'] ?? '-' }}
                        </div>
                    </div>
                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tahun
                            Berpisah / Meninggal</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-bold text-lg text-center">
                            {{ $dataPengajuan['tahun_berpisah'] ?? '-' }}
                        </div>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Alamat
                            Mantan Pasangan (Terakhir)</label>
                        <div
                            class="px-4 py-3 bg-orange-50 border border-orange-100 rounded-xl text-gray-800 font-semibold leading-relaxed">
                            {{ $dataPengajuan['alamat_mantan'] ?? '-' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-10">
                <h3 class="flex items-center gap-2 text-lg font-bold text-blue-600 border-b-2 border-gray-50 pb-3 mb-6">
                    <i class="fas fa-clipboard-check"></i> Pernyataan Pemohon
                </h3>
                <div class="bg-blue-50 p-6 rounded-xl border border-blue-200 flex items-start gap-4">
                    <i class="fas fa-check-circle text-2xl text-blue-600 mt-1"></i>
                    <p class="text-sm text-blue-900 font-medium leading-relaxed">
                        Pemohon menyatakan dengan sebenar-benarnya bahwa sejak tahun
                        <strong>{{ $dataPengajuan['tahun_berpisah'] ?? '....' }}</strong> telah berpisah/ditinggal
                        pasangan, dan <span class="font-bold underline">sampai saat ini berstatus belum menikah
                            lagi</span>.
                    </p>
                </div>
            </div>
                ...
                <div class="space-y-4">
                    ...
                    <div
                        ...
                        <div class="flex-1">
                            ...
                            <span
                                class="text-sm text-gray-800 font-medium">{{ !empty($dataPengajuan['file_bukti']) ? 'Berkas Tersedia' : 'Berkas Tidak Dilampirkan' }}</span>
                        </div>
                        @if(!empty($dataPengajuan['file_bukti']))
                            <a href="{{ asset('storage/' . $dataPengajuan['file_bukti']) }}" target="_blank"
                                class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                                <i class="fas fa-cloud-download-alt"></i> Lihat File
                            </a>
                        @endif
                ...
                <div class="bg-gray-50 rounded-2xl p-6 md:p-8 border border-gray-200 shadow-inner mt-12">
                    <div class="mb-6">
                        ...
                        <textarea name="keterangan" id="catatanPenolakan"
                            placeholder="Wajib diisi jika berkas ini ditolak..." rows="3"
