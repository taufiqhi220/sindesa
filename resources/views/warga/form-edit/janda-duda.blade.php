...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    <main class="flex-1 p-6 lg:p-10 overflow-x-hidden">
        ...
        <div class="max-w-3xl mx-auto text-center mb-10 mt-10">
            <h2 class="text-3xl font-bold text-[#1a5e35] mb-2">Edit Pengajuan Keterangan Janda / Duda</h2>
            <p class="text-gray-500">Perbaiki data administrasi yang salah pada pengajuan Anda sebelumnya.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] max-w-4xl mx-auto p-6 md:p-10">
            ...
            <form action="{{ route('warga.form.janda-duda.update', $surat->id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="number" name="nik_pemohon"
                            value="{{ old('nik_pemohon', $data['nik_pemohon'] ?? Auth::user()->nik) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_pemohon"
                            value="{{ old('nama_pemohon', $data['nama_pemohon'] ?? Auth::user()->name) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="tempat_lahir_pemohon"
                            value="{{ old('tempat_lahir_pemohon', $data['tempat_lahir_pemohon'] ?? Auth::user()->tempat_lahir) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                    <div>
                        ...
                        <input type="date" name="tanggal_lahir_pemohon"
                            value="{{ old('tanggal_lahir_pemohon', $data['tanggal_lahir_pemohon'] ?? Auth::user()->tanggal_lahir) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <select name="jenis_kelamin_pemohon"
                            ...
                            required>
                            @php $jk = old('jenis_kelamin_pemohon', $data['jenis_kelamin_pemohon'] ?? Auth::user()->jenis_kelamin); @endphp
                            <option value="" disabled>-- Pilih Jenis Kelamin --</option>
                            <option value="Laki-laki" {{ strcasecmp($jk, 'Laki-laki') == 0 || strcasecmp($jk, 'Laki-laki (Duda)') == 0 ? 'selected' : '' }}>
                                Laki-laki (Duda)</option>
                            <option value="Perempuan" {{ strcasecmp($jk, 'Perempuan') == 0 || strcasecmp($jk, 'Perempuan (Janda)') == 0 ? 'selected' : '' }}>
                                Perempuan (Janda)</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="penyebab_status"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm appearance-none transition-all"
                            required>
                            @php $penyebab = old('penyebab_status', $data['penyebab_status'] ?? ''); @endphp
                            <option value="" disabled>-- Pilih Alasan --</option>
                            <option value="Cerai Mati" {{ $penyebab == 'Cerai Mati' ? 'selected' : '' }}>Cerai Mati
                                (Meninggal)</option>
                            <option value="Cerai Hidup" {{ $penyebab == 'Cerai Hidup' ? 'selected' : '' }}>Cerai Hidup
                            </option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <textarea name="alamat_pemohon" rows="2"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                        required>{{ old('alamat_pemohon', $data['alamat_pemohon'] ?? Auth::user()->alamat_lengkap) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="nama_mantan"
                            value="{{ old('nama_mantan', $data['nama_mantan'] ?? '') }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Tahun Berpisah / Meninggal</label>
                        <input type="number" name="tahun_berpisah"
                            value="{{ old('tahun_berpisah', $data['tahun_berpisah'] ?? '') }}" min="1900" max="2099"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <label class="block mb-2 text-sm font-medium text-gray-800">Alamat Mantan Pasangan
                        (Terakhir)</label>
                    <textarea name="alamat_mantan" rows="2"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                        required>{{ old('alamat_mantan', $data['alamat_mantan'] ?? '') }}</textarea>
                </div>
                ...
                <div
                    ...
                    @if(isset($data['file_bukti']))
                        <div class="mb-4 p-3 bg-white border border-gray-200 rounded-xl flex items-center justify-between">
                            ...
                            <a href="{{ asset('storage/' . $data['file_bukti']) }}" target="_blank"
                                class="text-sm bg-blue-50 text-blue-600 hover:bg-blue-100 px-4 py-2 rounded-lg font-medium transition-colors">
                                <i class="fas fa-eye mr-1"></i> Lihat
                            </a>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <span id="name-bukti" class="text-sm text-gray-500 italic truncate">Tidak ada file baru
                            dipilih</span>
                    </div>
                </div>
