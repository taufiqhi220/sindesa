...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    <main class="flex-1 p-6 lg:p-10 overflow-x-hidden">
        ...
        <div class="max-w-3xl mx-auto text-center mb-10 mt-10">
            <h2 class="text-3xl font-bold text-[#1a5e35] mb-2">Edit Pengajuan Surat Keterangan Kehilangan</h2>
            <p class="text-gray-500">Perbaiki rincian laporan kehilangan barang/dokumen yang kurang tepat sebelumnya.
            </p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] max-w-4xl mx-auto p-6 md:p-10">
            ...
            <form action="{{ route('warga.form.kehilangan.update', $surat->id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="number" name="nik_pelapor"
                            value="{{ old('nik_pelapor', $data['nik_pelapor'] ?? Auth::user()->nik) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="tempat_lahir_pelapor"
                            value="{{ old('tempat_lahir_pelapor', $data['tempat_lahir_pelapor'] ?? Auth::user()->tempat_lahir) }}"
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_lahir_pelapor"
                            value="{{ old('tanggal_lahir_pelapor', $data['tanggal_lahir_pelapor'] ?? Auth::user()->tanggal_lahir) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                    <div>
                        ...
                        <select name="jenis_kelamin_pelapor"
                            ...
                            required>
                            @php $jk = old('jenis_kelamin_pelapor', $data['jenis_kelamin_pelapor'] ?? Auth::user()->jenis_kelamin); @endphp
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <select name="agama_pelapor"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm appearance-none transition-all"
                            required>
                            @php $agm = old('agama_pelapor', $data['agama_pelapor'] ?? Auth::user()->agama); @endphp
                    ...
                    <div>
                        ...
                        <input type="text" name="pekerjaan_pelapor"
                            value="{{ old('pekerjaan_pelapor', $data['pekerjaan_pelapor'] ?? Auth::user()->pekerjaan) }}"
                ...
                <div class="mb-8">
                    ...
                    <textarea name="alamat_pelapor" rows="2" placeholder="Dusun / Desa / Kecamatan"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                        required>{{ old('alamat_pelapor', $data['alamat_pelapor'] ?? Auth::user()->alamat_lengkap) }}</textarea>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="rincian_hilang" rows="3"
                        ...
                        required>{{ old('rincian_hilang', $data['rincian_hilang'] ?? '') }}</textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Waktu Kehilangan (Perkiraan)</label>
                        <input type="date" name="waktu_hilang"
                            value="{{ old('waktu_hilang', $data['waktu_hilang'] ?? '') }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-red-500 focus:ring-4 focus:ring-red-500/10 outline-none text-sm transition-all"
                            required>
                    </div>
                    <div>
                        ...
                        <input type="text" name="lokasi_hilang"
                            value="{{ old('lokasi_hilang', $data['lokasi_hilang'] ?? '') }}"
                ...
                <div
                    ...
                    @if(isset($data['file_bukti']) && $data['file_bukti'])
                        <div class="mb-4 p-3 bg-white border border-gray-200 rounded-xl flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center">
                                    <i class="fas fa-file-alt text-lg"></i>
                ...
                <div class="flex items-start gap-3 mt-10 mb-6 bg-red-50 p-4 rounded-xl border border-red-200">
                    ...
                    <label for="consent" class="text-sm text-gray-800 font-medium">
                        Dengan ini saya menyatakan bahwa laporan keterangan kehilangan yang saya berikan adalah benar,
                        dapat dipertanggungjawabkan, dan siap diproses ulang.
                    </label>
                </div>
