...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    <main class="flex-1 p-6 lg:p-10 overflow-x-hidden">
        ...
        <div class="max-w-3xl mx-auto text-center mb-10 mt-10">
            <h2 class="text-3xl font-bold text-[#1a5e35] mb-2">Edit Pengajuan Surat Pengantar Kartu Keluarga</h2>
            <p class="text-gray-500">Perbaiki data atau dokumen pengajuan pembuatan/pembaharuan KK Anda.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] max-w-4xl mx-auto p-6 md:p-10">
            ...
            <form action="{{ route('warga.form.kk.update', $surat->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <select name="tujuan_pengajuan"
                        ...
                        required>
                        @php $tujuan = old('tujuan_pengajuan', $data['tujuan_pengajuan'] ?? ''); @endphp
                        <option value="" disabled>-- Pilih Tujuan --</option>
                        <option value="Pembuatan Baru" {{ $tujuan == 'Pembuatan Baru' ? 'selected' : '' }}>Pembuatan Baru
                        </option>
                        <option value="Pembaharuan" {{ $tujuan == 'Pembaharuan' || strcasecmp($tujuan, 'Pembaharuan / Perubahan Data') == 0 ? 'selected' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Nomor Kartu Keluarga (Jika
                            ada)</label>
                        <input type="number" name="kk_lama"
                            value="{{ old('kk_lama', $data['kk_lama'] ?? Auth::user()->no_kk) }}"
                ...
                <div class="mb-6">
                    <label class="block mb-2 text-sm font-medium text-gray-800">Nama Kepala Keluarga (Tujuan)</label>
                ...
                <div
                    ...
                    <label class="block text-sm font-semibold text-gray-800 mb-1">Upload KK Lama / Surat
                        Kehilangan</label>
                    <span class="block text-xs text-gray-500 mb-4">Jika pembaharuan KK wajib lampirkan KK Lama. Format:
                        PDF, JPG, PNG (Maks 5MB).</span>

                    @if(isset($data['file_kk_lama']))
                        <div class="mb-4 p-3 bg-white border border-gray-200 rounded-xl flex items-center justify-between">
                            ...
                            <a href="{{ asset('storage/' . $data['file_kk_lama']) }}" target="_blank"
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <span id="name-kk-lama" class="text-sm text-gray-500 italic truncate">Tidak ada file baru
                            dipilih</span>
                    </div>
                </div>
                ...
                <div
                    class="mb-6 border-2 border-dashed border-gray-300 p-6 rounded-2xl bg-gray-50 hover:border-[#1a5e35] transition-colors">
                    <label class="block text-sm font-semibold text-gray-800 mb-1">Upload Buku Nikah / Akta Cerai (Jika
                        Ada)</label>
                    <span class="block text-xs text-gray-500 mb-4">Diperlukan jika status perkawinan berubah. Format:
                        PDF, JPG, PNG (Maks 5MB).</span>

                    @if(isset($data['file_nikah']) && $data['file_nikah'])
                        <div class="mb-4 p-3 bg-white border border-gray-200 rounded-xl flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center">
                                    <i class="fas fa-file-alt text-lg"></i>
                            ...
                            <a href="{{ asset('storage/' . $data['file_nikah']) }}" target="_blank"
                                class="text-sm bg-blue-50 text-blue-600 hover:bg-blue-100 px-4 py-2 rounded-lg font-medium transition-colors">
                                <i class="fas fa-eye mr-1"></i> Lihat
                            </a>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <span id="name-nikah" class="text-sm text-gray-500 italic truncate">Tidak ada file baru
                            dipilih</span>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-3 mt-10 mb-6 bg-yellow-50 p-4 rounded-xl border border-yellow-200">
                    ...
                    <label for="consent" class="text-sm text-gray-700">Dengan ini saya menyatakan data yang saya
                        perbarui adalah benar dan siap diproses kembali.</label>
                </div>
