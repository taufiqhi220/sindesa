...
<head>
    ...
    <title>Pengajuan Belum Menikah - SINDESA</title>
...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    <main class="flex-1 p-6 lg:p-10 overflow-x-hidden">
        ...
        <div class="max-w-3xl mx-auto text-center mb-10 mt-10">
            <h2 class="text-3xl font-bold text-[#1a5e35] mb-2">Edit Pengajuan Belum Menikah</h2>
            <p class="text-gray-500">Silakan perbaiki data yang salah. Kosongkan upload file jika tidak ingin mengubah
                dokumen.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] max-w-4xl mx-auto p-6 md:p-10">
            ...
            <form action="{{ route('warga.form.belum-menikah.update', $surat->id) }}" method="POST"
                ...
                <h3 class="text-lg font-semibold text-[#1a5e35] border-b-2 border-gray-100 pb-2 mb-6">Data Pemohon
                    (Terkunci)</h3>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">NIK Pemohon</label>
                        <input type="number" name="nik_pemohon" value="{{ $data['nik_pemohon'] ?? Auth::user()->nik }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 outline-none text-sm"
                            readonly required>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Nama Lengkap</label>
                        <input type="text" name="nama_pemohon" value="{{ $data['nama_pemohon'] ?? Auth::user()->name }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 outline-none text-sm"
                            readonly required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="tempat_lahir_pemohon"
                            value="{{ $data['tempat_lahir_pemohon'] ?? Auth::user()->tempat_lahir }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 outline-none text-sm"
                            readonly required>
                    </div>
                    <div>
                        ...
                        <input type="date" name="tanggal_lahir_pemohon"
                            value="{{ $data['tanggal_lahir_pemohon'] ?? Auth::user()->tanggal_lahir }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 outline-none text-sm"
                            readonly required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="jenis_kelamin_pemohon"
                            value="{{ $data['jenis_kelamin_pemohon'] ?? Auth::user()->jenis_kelamin }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 outline-none text-sm"
                            readonly required>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Agama</label>
                        <input type="text" name="agama_pemohon"
                            value="{{ $data['agama_pemohon'] ?? Auth::user()->agama }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 outline-none text-sm"
                            readonly required>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="pekerjaan_pemohon"
                        value="{{ $data['pekerjaan_pemohon'] ?? Auth::user()->pekerjaan }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 outline-none text-sm"
                        readonly required>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <textarea name="alamat_pemohon" rows="2"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl bg-gray-50 outline-none text-sm"
                        readonly required>{{ $data['alamat_pemohon'] ?? Auth::user()->alamat_lengkap }}</textarea>
                </div>

                <h3 class="text-lg font-semibold text-[#1a5e35] border-b-2 border-gray-100 pb-2 mb-6">Data Bapak Kandung
                </h3>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">NIK Bapak</label>
                        <input type="number" name="nik_bapak" value="{{ old('nik_bapak', $data['nik_bapak'] ?? '') }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm"
                            required>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Nama Bapak</label>
                        <input type="text" name="nama_bapak" value="{{ old('nama_bapak', $data['nama_bapak'] ?? '') }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm"
                            required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Tempat Lahir</label>
                        <input type="text" name="tempat_lahir_bapak"
                            value="{{ old('tempat_lahir_bapak', $data['tempat_lahir_bapak'] ?? '') }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm"
                            required>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Tanggal Lahir</label>
                        <input type="date" name="tanggal_lahir_bapak"
                            value="{{ old('tanggal_lahir_bapak', $data['tanggal_lahir_bapak'] ?? '') }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm"
                            required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Agama</label>
                        <select name="agama_bapak"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm appearance-none"
                            required>
                            @php $agm_b = old('agama_bapak', $data['agama_bapak'] ?? ''); @endphp
                            <option value="Islam" {{ $agm_b == 'Islam' ? 'selected' : '' }}>Islam</option>
                            <option value="Kristen Protestan" {{ $agm_b == 'Kristen Protestan' ? 'selected' : '' }}>
                                Kristen Protestan</option>
                            <option value="Katolik" {{ $agm_b == 'Katolik' ? 'selected' : '' }}>Katolik</option>
                            <option value="Hindu" {{ $agm_b == 'Hindu' ? 'selected' : '' }}>Hindu</option>
                            <option value="Buddha" {{ $agm_b == 'Buddha' ? 'selected' : '' }}>Buddha</option>
                            <option value="Konghucu" {{ $agm_b == 'Konghucu' ? 'selected' : '' }}>Konghucu</option>
                        </select>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Pekerjaan</label>
                        <input type="text" name="pekerjaan_bapak"
                            value="{{ old('pekerjaan_bapak', $data['pekerjaan_bapak'] ?? '') }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm"
                            required>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <label class="block mb-2 text-sm font-medium text-gray-800">Alamat Lengkap Bapak</label>
                    <textarea name="alamat_bapak" rows="2"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm"
                        required>{{ old('alamat_bapak', $data['alamat_bapak'] ?? '') }}</textarea>
                </div>

                <h3 class="text-lg font-semibold text-[#1a5e35] border-b-2 border-gray-100 pb-2 mb-6">Data Ibu Kandung
                </h3>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">NIK Ibu</label>
                        <input type="number" name="nik_ibu" value="{{ old('nik_ibu', $data['nik_ibu'] ?? '') }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm"
                            required>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Nama Ibu</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <select name="agama_ibu"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm appearance-none"
                            required>
                            @php $agm_i = old('agama_ibu', $data['agama_ibu'] ?? ''); @endphp
                            <option value="Islam" {{ $agm_i == 'Islam' ? 'selected' : '' }}>Islam</option>
                            <option value="Kristen Protestan" {{ $agm_i == 'Kristen Protestan' ? 'selected' : '' }}>
                                Kristen Protestan</option>
                            <option value="Katolik" {{ $agm_i == 'Katolik' ? 'selected' : '' }}>Katolik</option>
                            <option value="Hindu" {{ $agm_i == 'Hindu' ? 'selected' : '' }}>Hindu</option>
                            <option value="Buddha" {{ $agm_i == 'Buddha' ? 'selected' : '' }}>Buddha</option>
                            <option value="Konghucu" {{ $agm_i == 'Konghucu' ? 'selected' : '' }}>Konghucu</option>
                        </select>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Pekerjaan</label>
                ...
                <div class="mb-8">
                    <label class="block mb-2 text-sm font-medium text-gray-800">Alamat Lengkap Ibu</label>
                    <textarea name="alamat_ibu" rows="2"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] outline-none text-sm"
                        required>{{ old('alamat_ibu', $data['alamat_ibu'] ?? '') }}</textarea>
                </div>
                ...
                <div
                    ...
                    <label class="block text-sm font-semibold text-gray-800 mb-1">Upload KTP Orang Tua
                        (Opsional)</label>
                    ...
                    @if(isset($data['file_lain']) && $data['file_lain'])
                        <div class="mb-4 p-3 bg-white border border-gray-200 rounded-xl flex items-center justify-between">
                            ...
                            <a href="{{ asset('storage/' . $data['file_lain']) }}" target="_blank"
                                class="text-sm bg-blue-50 text-blue-600 hover:bg-blue-100 px-4 py-2 rounded-lg font-medium transition-colors">
                                <i class="fas fa-eye mr-1"></i> Lihat
                            </a>
                        </div>
                ...
                <div class="flex items-start gap-3 mt-10 mb-6 bg-yellow-50 p-4 rounded-xl border border-yellow-200">
                    <input type="checkbox" id="consent" oninvalid="this.setCustomValidity('Harap centang kotak persetujuan ini sebelum melanjutkan.')" oninput="this.setCustomValidity('')" name="consent"
                        ...
                        required>
                    <label for="consent" class="text-sm text-gray-700">Dengan ini saya menyatakan bahwa data yang
                        diperbarui adalah benar, saya sungguh berstatus belum menikah, dan siap diproses
                        kembali.</label>
                </div>
