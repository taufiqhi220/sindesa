...
<head>
    ...
    <title>Pengajuan Keterangan Usaha - SINDESA</title>
...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    <main class="flex-1 p-6 lg:p-10 overflow-x-hidden">
        ...
        <div class="max-w-3xl mx-auto text-center mb-10 mt-10">
            <h2 class="text-3xl font-bold text-[#1a5e35] mb-2">Edit Pengajuan Surat Keterangan Usaha</h2>
            <p class="text-gray-500">Perbaiki data kepemilikan atau alamat usaha Anda yang kurang tepat.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] max-w-4xl mx-auto p-6 md:p-10">
            ...
            <form action="{{ route('warga.form.usaha.update', $surat->id) }}" method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="no_kk"
                            value="{{ old('no_kk', $data['no_kk'] ?? Auth::user()->no_kk) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Alamat Lengkap (Dusun /
                            Jalan)</label>
                        <input type="text" name="alamat_dusun"
                            value="{{ old('alamat_dusun', $data['alamat_dusun'] ?? '') }}"
                            placeholder="Contoh: Maung, Dusun Kamali"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                    <div class="flex gap-4">
                        <div class="w-1/2">
                            ...
                            <input type="number" name="rt" value="{{ old('rt', $data['rt'] ?? '') }}" placeholder="000"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                                required>
                        </div>
                        <div class="w-1/2">
                            <label class="block mb-2 text-sm font-medium text-gray-800">RW</label>
                            <input type="number" name="rw" value="{{ old('rw', $data['rw'] ?? '') }}" placeholder="000"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                                required>
                        </div>
                    </div>
                ...
                <h3 class="text-lg font-semibold text-[#1a5e35] border-b-2 border-gray-100 pb-2 mb-6">Keterangan Usaha
                </h3>

                <div class="mb-6">
                    <label class="block mb-2 text-sm font-medium text-gray-800">Jenis Usaha Utama</label>
                    <input type="text" name="jenis_usaha" value="{{ old('jenis_usaha', $data['jenis_usaha'] ?? '') }}"
                        placeholder="Contoh: JUAL BARANG CAMPURAN"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                        required>
                </div>

                <div class="mb-6">
                    <label class="block mb-2 text-sm font-medium text-gray-800">Jenis Usaha Sampingan (Opsional)</label>
                    <input type="text" name="usaha_sampingan"
                        value="{{ old('usaha_sampingan', $data['usaha_sampingan'] ?? '') }}"
                        placeholder="Isi tanda strip (-) jika tidak ada"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all">
                </div>

                <div class="mb-8">
                    <label class="block mb-2 text-sm font-medium text-gray-800">Alamat Tempat Usaha Beroperasi</label>
                    <textarea name="alamat_usaha" rows="2" placeholder="Contoh: MAUNG DESA BUTTU SAWE"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                ...
                <div
                    ...
                    <label class="block text-sm font-semibold text-gray-800 mb-1">Foto Tempat Usaha (Opsional)</label>
                    <span class="block text-xs text-gray-500 mb-4">Maksimal 5MB (JPG/PNG).</span>

                    @if(isset($data['file_foto_usaha']) && $data['file_foto_usaha'])
                        <div class="mb-4 p-3 bg-white border border-gray-200 rounded-xl flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-gray-800">Foto Saat Ini</p>
                                    <p class="text-xs text-gray-500">Telah tersimpan</p>
                                </div>
                            </div>
                            <a href="{{ asset('storage/' . $data['file_foto_usaha']) }}" target="_blank"
                                class="text-sm bg-blue-50 text-blue-600 hover:bg-blue-100 px-4 py-2 rounded-lg font-medium transition-colors">
                                <i class="fas fa-eye mr-1"></i> Lihat
                            </a>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        <input type="file" id="file-usaha" name="file_foto_usaha" class="hidden"
                            onchange="updateFileName(this, 'name-usaha')">
                        <label for="file-usaha"
                            class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full cursor-pointer font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                            <i class="fas fa-cloud-upload-alt"></i> Ganti File
                        </label>
                        <span id="name-usaha" class="text-sm text-gray-500 italic truncate">Tidak ada file baru
                            dipilih</span>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-3 mt-10 mb-6 bg-yellow-50 p-4 rounded-xl border border-yellow-200">
                    <input type="checkbox" id="consent" oninvalid="this.setCustomValidity('Harap centang kotak persetujuan ini sebelum melanjutkan.')" oninput="this.setCustomValidity('')" name="consent"
                        ...
                        required>
                    <label for="consent" class="text-sm text-gray-700">Dengan ini saya menyatakan bahwa data usaha di
                        atas adalah benar milik saya dan siap diproses ulang.</label>
                </div>
