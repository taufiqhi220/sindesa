...
<head>
    ...
    <title>Pengajuan Keterangan Penghasilan - SINDESA</title>
...
<body class="font-['Poppins'] bg-[#f4f6f9] flex min-h-screen">
    ...
    <main class="flex-1 p-6 lg:p-10 overflow-x-hidden">
        ...
        <div class="max-w-3xl mx-auto text-center mb-10 mt-10">
            <h2 class="text-3xl font-bold text-[#1a5e35] mb-2">Edit Surat Keterangan Penghasilan</h2>
            <p class="text-gray-500">Perbaiki data atau dokumen rincian penghasilan yang kurang tepat pada pengajuan
                sebelumnya.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_5px_20px_rgba(0,0,0,0.05)] max-w-4xl mx-auto p-6 md:p-10">
            ...
            <form action="{{ route('warga.form.penghasilan.update', $surat->id) }}" method="POST"
                ...
                <h3 class="text-lg font-semibold text-[#1a5e35] border-b-2 border-gray-100 pb-2 mb-6">Identitas Orang
                    Tua / Wali</h3>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="pekerjaan"
                            ...
                            placeholder="Contoh: Petani / Mengurus Rumah Tangga"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                </div>
                ...
                <h3 class="text-lg font-semibold text-[#1a5e35] border-b-2 border-gray-100 pb-2 mb-6">Rincian
                    Penghasilan & Tanggungan</h3>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Jumlah Penghasilan / Bulan</label>
                        <input type="text" name="jumlah_penghasilan"
                            value="{{ old('jumlah_penghasilan', $data['jumlah_penghasilan'] ?? '') }}"
                            placeholder="Contoh: Rp 1.000.000"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-800">Jumlah Tanggungan Keluarga</label>
                        <input type="number" name="jumlah_tanggungan"
                            value="{{ old('jumlah_tanggungan', $data['jumlah_tanggungan'] ?? '') }}"
                            placeholder="Berapa Orang" min="0"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                            required>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <label class="block mb-2 text-sm font-medium text-gray-800">Nama Anak / Anggota Keluarga yang
                        Ditanggung</label>
                    <input type="text" name="nama_tanggungan"
                        value="{{ old('nama_tanggungan', $data['nama_tanggungan'] ?? '') }}"
                        placeholder="Nama Lengkap Anak (Misal: Untuk keperluan beasiswa anak)"
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:border-[#1a5e35] focus:ring-4 focus:ring-[#1a5e35]/10 outline-none text-sm transition-all"
                        required>
                </div>
                ...
                <div
                    ...
                    <label class="block text-sm font-semibold text-gray-800 mb-1">Upload KTP & KK Orang Tua</label>
                    <span class="block text-xs text-gray-500 mb-4">Maksimal 5MB (PDF/JPG/PNG). Bisa digabung 1 file atau
                        zip.</span>
                ...
                <div
                    ...
                    <label class="block text-sm font-semibold text-gray-800 mb-1">Upload Bukti / KTP Anak
                        (Opsional)</label>
                    <span class="block text-xs text-gray-500 mb-4">Maksimal 5MB (PDF/JPG/PNG).</span>

                    @if(isset($data['file_anak']) && $data['file_anak'])
                        <div class="mb-4 p-3 bg-white border border-gray-200 rounded-xl flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center">
                                    <i class="fas fa-file-alt text-lg"></i>
                            ...
                            <a href="{{ asset('storage/' . $data['file_anak']) }}" target="_blank"
                                class="text-sm bg-blue-50 text-blue-600 hover:bg-blue-100 px-4 py-2 rounded-lg font-medium transition-colors">
                                <i class="fas fa-eye mr-1"></i> Lihat
                            </a>
                        </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        <input type="file" id="file-anak" name="file_anak" class="hidden"
                            onchange="updateFileName(this, 'name-anak')">
                        <label for="file-anak"
                            class="inline-flex justify-center items-center gap-2 px-6 py-2.5 bg-white border border-[#1a5e35] text-[#1a5e35] rounded-full cursor-pointer font-semibold text-sm hover:bg-[#1a5e35] hover:text-white transition-all shadow-sm">
                            <i class="fas fa-cloud-upload-alt"></i> Ganti File
                        </label>
                        <span id="name-anak" class="text-sm text-gray-500 italic truncate">Tidak ada file baru
                            dipilih</span>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-3 mt-10 mb-6 bg-red-50 p-4 rounded-xl border border-red-200">
                    ...
                    <label for="consent" class="text-sm text-gray-800 font-medium">
                        Saya menyatakan bahwa rincian penghasilan tersebut adalah benar dan saya siap diproses kembali.
                        Apabila di kemudian hari ditemukan ketidakbenaran, saya siap mempertanggungjawabkannya.
                    </label>
                </div>
